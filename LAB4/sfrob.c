...
int main(void) {
    ...
    while (current != EOF && !ferror(stdin)) { //read file untill end of word
        holdWord[charIterator] = current;
        ...
        if (current == ' ') { //when hit the end of the word
        ...
        else if (current == ' ' && next == ' ') { //when extra space
            while(current == ' ') {
                current = getchar();
                errorCheck();
            }
        ...
        current = next; //when current is next
        next = getchar();
        errorCheck();
        charIterator++;
