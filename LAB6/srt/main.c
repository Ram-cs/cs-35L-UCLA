...
main( int argc, char **argv )
{
    ...
    //ID for threads
    int idForThread[nthreads];
    //make a thread
    pthread_t isThread[nthreads];
    
    //to use for nthreads
    pthread_t* getThread = (pthread_t *)malloc(nthreads * sizeof(pthread_t));
    //index for array
    int* index = (int*)malloc(nthreads * sizeof(int));
    
    //if the pointer is NULL, memory could't allocate
    if(getThread == NULL)
    {
    ...
    //pointer is null, memory culdn't allocate
    if(index == NULL)
    {
        fprintf(stderr, "No memeroy allocation");
        exit(1);
    }
    ...
    while (t < nthreads) {
        idForThread[t] = t;
        int temp = pthread_create(&isThread[t], NULL, imageAndClr,&idForThread[t]);
        if(temp)
        {
    ...
    //after creation of threads, needs to free the thread
    
    int i = 0;
    while (i < nthreads) {
        int temp = pthread_join(isThread[i], NULL);
        if(temp) //if temp is true, means can not free the thread
        {
            fprintf(stderr, "Can not free thread");
            exit(-1);
        }
        i++;
    ...
    for(int x = 0; x < width; x++)
    {
        ...
        {
            printf("%.0f %.0f %.0f\n",
                   printVals[x][y][0], printVals[x][y][1], printVals[x][y][2]);
        }
        printf("\n");
    }
